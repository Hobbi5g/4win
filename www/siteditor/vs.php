...
<?
	...
	//$base->BaseSelect("SELECT * FROM searchdata WHERE searchdata_id=".$wid);
	$sSQL = "SELECT * FROM searchdata WHERE searchdata_id='".$wid."'";
	$row = $conn->getRow($sSQL);
	//$row = $base->BaseFetch();
	//$base->BaseFinish();
	$games = unserialize($row['relevant_docs']);
	arsort($games);
?>
<pre>
<?=$row['stemmed_word']?>, <?=count($games)?> title(s) found<br>
<? print_r($games);?>
</pre>
